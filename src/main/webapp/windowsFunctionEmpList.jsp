...
<%
	...
	/*
	select rownum 번호, employee_id 아이디, last_name 이름, salary 급여, 
	...
	*/
	PreparedStatement stmt = null;
  		ResultSet rs = null;
	String sql = "SELECT 번호, 아이디, 이름, 급여, 전체급여평균, 전체급여합계, 전체사원수 FROM(SELECT rownum 번호, employee_id 아이디, last_name 이름, salary 급여, round(avg(salary) over()) 전체급여평균, sum(salary) over() 전체급여합계, count(*) over() 전체사원수 FROM employees) WHERE 번호 BETWEEN ? AND ?";
	...
	while(rs.next()) {
		...
		e.put("전체급여평균", rs.getInt("전체급여평균"));
		e.put("전체급여합계", rs.getInt("전체급여합계"));
		e.put("전체사원수", rs.getInt("전체사원수"));
		list.add(e);
	}
...
<h1>WindowsFunctionEmpList</h1>
	<table border="1">
		...
		<tr>
			...
			<td><%=(Integer)e.get("전체급여평균")%></td>
			<td><%=(Integer)e.get("전체급여합계")%></td>
			<td><%=(Integer)e.get("전체사원수")%></td>
		</tr>
		<%
			}
		%>
	...
	%>
		<a href="<%=request.getContextPath()%>/windowsFunctionEmpList.jsp?currentPage=<%=minPageNum - pagePerPageNum%>">이전</a>
	<%
		}
	...
	%>	
		<span><a href="<%=request.getContextPath()%>/windowsFunctionEmpList.jsp?currentPage=<%=i%>"><%=i%></a>&nbsp;</span>
	<%
	...
	%>
		<a href="<%=request.getContextPath()%>/windowsFunctionEmpList.jsp?currentPage=<%=minPageNum + pagePerPageNum%>">다음</a>
	<%
		}
	%>
</body>
